...
export const updateProfile = connectDBAction(async (prevState: unknown, formData: FormData) => {
  ...
  if (ctx.userId === process.env.EXAMPLE_USER_ID) {
    return errorResponseAction('Update profile is not possible for this user');
  }
  ...
  await usersCol.findOneAndUpdate({ _id: new ObjectId(ctx.userId) }, { $set: { ...parsedData } });
