...
export const editAdvert = connectDBAction(async (prevState: unknown, formData: FormData) => {
  ...
  const parsedData = AdvertSchema.extend({ id: z.string() }).omit({ views: true }).parse(dataForm);
